{ config, pkgs, lib, ... }: {
  ...
  environment.systemPackages = with pkgs; [neovim git fzf tmux];
  environment.userPackages = with pkgs; [ emacs-nox zsh cowsay haskell docker sbcl htop tree nix-tree];
  security.acme = {
    acceptTerms = true;
